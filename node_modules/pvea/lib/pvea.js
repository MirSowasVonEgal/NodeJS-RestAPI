...
class pvea {
  ...
  async createAccessTicket(param) {
    await this.authCheck()
    param.password = (this.ticket).split("=")[1] + "==";
    return await this.post('/access/ticket', param);
  }

  async createAccessUser(param) {
    return await this.post('/access/users', param);
  }

  async putAccessAcl(param) {
    return await this.put('/access/acl', param);
  }
  ...
  async restoreBackup(node, param) {
    return await this.post(`/nodes/${node}/lxc`, param)
  }

  async deleteBackup(node, volume, volid) {
    return await this.delete(`/nodes/${node}/storage/${volume}/content/${volid}?delay=5`)
  }
  ...
  async getBackups(node, volume) {
    return await this.get(`/nodes/${node}/storage/${volume}/content?content=backup`)
  }
  ...
  // https://pve.proxmox.com/pve-docs/api-viewer/index.html#/nodes/{node}/lxc/{vmid}/config
  async setLxcConfig(node, vmid, param) {
    return await this.put(`/nodes/${node}/lxc/${vmid}/config`, param)
  }
  ...
  async createLxcVncProxy(node, vmid, param) {
    return await this.post(`/nodes/${node}/lxc/${vmid}/vncproxy`, param)
  }
  ...
  async resumeQemuVm(node, vmid) {
    return await this.post(`/nodes/${node}/qemu/${vmid}/status/resume`)
  }
  ...
  async getClusterTasks(){
    return await this.get('/cluster/tasks')
  }
  async getNextVMID(){
